import type {HexString, Proof, RPCEthGetBlock, RPCEthGetProof} from '../types.js';
import {ethers} from 'ethers';
import {AbstractCommit, AbstractGateway, ABI_CODER, encodeProofV1, type GatewayConstructor} from './AbstractGateway.js';

export const L2ToL1MessagePasser = '0x4200000000000000000000000000000000000016';

export type AbstractOPGatewayConstructor = GatewayConstructor & {L2ToL1MessagePasser?: HexString};
...
const OutputRootProofType = 'tuple(bytes32 version, bytes32 stateRoot, bytes32 messagePasserStorageRoot, bytes32 latestBlockhash)';
...
export abstract class AbstractOPGateway extends AbstractGateway<OPCommit> {
	...
	override encodeWitness(commit: OPCommit, proofs: Proof[], order: Uint8Array): HexString {
		return ABI_CODER.encode(
			[OutputRootProofType, 'bytes[][]', 'bytes'],
			[commit.rootProof(), proofs, order]
		);
	}
	override encodeWitnessV1(commit: OPCommit, accountProof: Proof, storageProofs: Proof[]): HexString {
		return ABI_CODER.encode(
			[`tuple(uint256 outputIndex, ${OutputRootProofType})`, 'tuple(bytes, bytes[])'],
