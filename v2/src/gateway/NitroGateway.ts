...
export class NitroGateway extends AbstractGateway<NitroCommit> {
	...
	constructor(args: GatewayConstructor & NitroGatewayConstructor) {
		super(args);
		this.L2Rollup = new ethers.Contract(args.L2Rollup, [	
	...
	override encodeWitness(commit: NitroCommit, proofs: Proof[], order: Uint8Array) {
		return ABI_CODER.encode(
			['bytes32', 'bytes', 'bytes[][]', 'bytes'], 
			[commit.sendRoot, commit.rlpEncodedBlock, proofs, order]
		);
	}
	override encodeWitnessV1(commit: NitroCommit, accountProof: Proof, storageProofs: Proof[]) {
		return ABI_CODER.encode(
			['tuple(bytes32 version, bytes32 sendRoot, uint64 nodeIndex, bytes rlpEncodedBlock)', 'tuple(bytes, bytes[])'], 
	...
	override async fetchCommit(index: number) {
		...
		return new NitroCommit(index, json.number, blockHash, sendRoot, rlpEncodedBlock);
	}
}
