import type { HexString, EncodedProof, Provider, ChainPair } from './types.js';
...
export type AbstractGatewayConstructor = {
  provider1: Provider;
  provider2: Provider;
  errorRetryMs?: number;
...
export type GatewayConfig<T> = T &
  ChainPair &
  Partial<AbstractGatewayConstructor>;
...
> extends EZCCIP {
  ...
  readonly blockDelayV1;
  readonly commitStep;
  readonly commitOffset;
  readonly callCache;
  readonly activeCommitCache;
  readonly recentCommits;
  readonly latestCache;
  readonly delayedCache;
  ...
  constructor({
    ...
    blockDelay = 10, // offset from head for "latest" commit
  ...
  }: AbstractGatewayConstructor) {
    super();
    this.provider1 = provider1;
    this.provider2 = provider2;
    this.writeCommitMs = writeCommitMs;
    this.blockDelayV1 = blockDelay;
    ...
    this.latestCache = new CachedValue(
      async () => Number(await this.fetchLatestCommitIndex(0)),
      checkCommitMs,
      errorRetryMs
    );
    this.delayedCache = new CachedValue(
      async () => Number(await this.fetchLatestCommitIndex(this.blockDelayV1)),
      checkCommitMs,
      errorRetryMs
    );
    this.recentCommits = Array<C | undefined>(commitDepth); // circular buffer
    this.activeCommitCache = new CachedMap<number, C>({
      cacheMs: 0,
      errorMs: errorRetryMs,
      maxCached: 2 * commitDepth,
    });
    this.callCache = new CachedMap<string, Uint8Array>({
      cacheMs: Infinity,
    ...
    this.register(
      ...
      async ([ctx, { ops, inputs }], _context, history) => {
        ...
        history.show = [index, ethers.hexlify(ops), hash];
  ...
  get supportedWindow() {
    return this.commitDepth * this.commitStep;
  }
  effectiveCommitDelay(blockDelay: number) {
    return Math.ceil((blockDelay * 12000) / this.writeCommitMs);
  }
  get commitParams() {
    const {
      commitStep,
    ...
    return {
      ...
      supportedWindow,
      blockDelayV1,
      effectiveCommitDelayV1: this.effectiveCommitDelay(blockDelayV1),
    };
  }
  ...
  abstract fetchLatestCommitIndex(blockDelay: number): Promise<number>;
