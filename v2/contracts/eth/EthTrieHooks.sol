...
library EthTrieHooks {
	...
	function proveAccountState(bytes32 stateRoot, address target, bytes memory proof) internal pure returns (bytes32 storageRoot) {
		...
		bytes32 codeHash = RLPReaderExt.strictBytes32FromRLP(accountState[3]);
		return codeHash == NULL_CODE_HASH ? NOT_A_CONTRACT : RLPReaderExt.strictBytes32FromRLP(accountState[2]);
	}
