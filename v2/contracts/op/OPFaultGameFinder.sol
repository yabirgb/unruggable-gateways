...
interface IDisputeGameFactory {
	...
	function gameAtIndex(uint256 index) external view returns (uint256 gameType, uint256 created, IDisputeGame gameProxy);
}
...
contract OPFaultGameFinder {
	...
	function findFinalizedGameIndex(IOptimismPortal portal, uint256 gameTypes, uint256 i) external view virtual returns (uint256) {
	...
	function getFinalizedGame(IOptimismPortal portal, uint256 gameTypes, uint256 gameIndex) external view returns (uint256 gameType, IDisputeGame gameProxy, uint256 l2BlockNumber) {
		if (gameTypes == 0) gameTypes = 1 << portal.respectedGameType();
		IDisputeGameFactory factory = portal.disputeGameFactory();
	...
	function _isFinalizedGame(uint256 gameTypes, uint256 gt, IDisputeGame gameProxy) internal view returns (bool) {
		return (gameTypes & (1 << gt)) != 0 && gameProxy.status() == DEFENDER_WINS;
	}
