import {dnsEncode, namehash, ZeroAddress, ZeroHash, AbiCoder, toUtf8String} from 'ethers';
...
let registry_root = await deployRegistry(ZeroAddress);
let registry_eth  = await deployRegistry(registry_root.target);

await foundry.confirm(storage.setRegistry(ZeroAddress, ZeroHash, registry_root))
await foundry.confirm(registry_root.setSubnode(wallet_admin, dns('eth'), resolver_eth, registry_eth, wallet_admin));
await foundry.confirm(registry_eth.setSubnode(wallet_raffy, dns('raffy.eth'), resolver_raffy, ZeroAddress, wallet_raffy));

await foundry.confirm(resolver_eth.setText(namehash('chonk.eth'), 'name', 'Chonk'));
await foundry.confirm(resolver_raffy.setText(namehash('raffy.eth'), 'name', 'Raffy'));
await foundry.confirm(resolver_raffy.setText(namehash('sub.raffy.eth'), 'name', 'Subdomain!'));
