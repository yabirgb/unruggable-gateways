import { NitroGateway } from '../../src/nitro/NitroGateway.js';
...
describe('arb1', async () => {
  const config = NitroGateway.arb1MainnetConfig;
  const foundry = await Foundry.launch({
    fork: providerURL(config.chain1),
    infoLog: false,
  });
  afterAll(() => foundry.shutdown());
  const gateway = new NitroGateway({
  ...
  const verifier = await foundry.deploy({
    file: 'NitroVerifier',
    args: [[ccip.endpoint], gateway.supportedWindow, gateway.L2Rollup],
  });
