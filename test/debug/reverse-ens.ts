...
async function primary(address: HexAddress) {
	...
	req.setSlot(0).pushInput(iNode).follow();
	req.offset(1).read();
	// pick resolver
	...
	req.setSlot(1).pushInput(iNode).follow().readBytes();
	// https://etherscan.io/address/0x231b0Ee14048e9dCcD1d247744d114a4EB5E8E63
	req.setSlot(1).pushInput(iNode).follow().read();
   	req.setSlot(8).follow().pushInput(iNode).follow().readBytes();
	// pick name()
